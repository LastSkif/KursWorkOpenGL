...
public class TableRenderer implements GLSurfaceView.Renderer {
    ...
    private Shaders mSimpleShadowProgram;
    ...
    /*объекты на экране*/
    private ObjectsOnTable Cup;
    private ObjectsOnTable Table;
    private ObjectsOnTable Apple;
    private ObjectsOnTable Coffee;
    private ObjectsOnTable Banana;
    private ObjectsOnTable Pear;
    private ObjectsOnTable Satchel;

    TableRenderer(Context c) {
        this.c = c;
    }
    ...
    public void onSurfaceCreated(GL10 unused, EGLConfig config) {
        ...
        Table = new ObjectsOnTable(c, new float[]{0.6f, 0.5f, 0.4f, 1.0f}, "TableAndLegs.obj", false);
        Apple = new ObjectsOnTable(c, new float[]{1.0f, 0.2f, 0f, 1.0f}, "NoPine.obj", false);
        Banana = new ObjectsOnTable(c, new float[]{1.0f, 1.0f, 0.0f, 1.0f}, "Banana.obj", true);
        Satchel = new ObjectsOnTable(c, new float[]{0.95f, 0.35f, 0.5f, 1.0f}, "Satchel.obj", false);
        Pear = new ObjectsOnTable(c, new float[]{1.0f, 1.0f, 0.0f, 1.0f}, "Pear.obj", true);
        Coffee = new ObjectsOnTable(c, new float[]{0.45f, 0.4f, 0.3f, 1.0f}, "Coffee.obj", false);
        Cup = new ObjectsOnTable(c, new float[]{0.0f, 0.45f, 0.65f, 1.0f}, "CoffeeCup.obj", true);
        ...
        String depth_tex_f_with_simple_shadow_shader = "precision mediump float;\n" +
                ...
                "}  ";
        mSimpleShadowProgram = new Shaders(depth_tex_v_with_shadow_shader, depth_tex_f_with_simple_shadow_shader);
    ...
    public void onSurfaceChanged(GL10 unused, int width, int height)//изменяем положение объектов
    {
        ...
        Matrix.frustumM(mProjectionMatrix, 0, -ratio, ratio, -1f, 1f, 1.0f, 100.0f);
    ...
    private void draw_picture()//отрисовываем объекты
    {
        ...
        Coffee.render(scene_positionAttribute, scene_normalAttribute, scene_colorAttribute);
        Banana.render(scene_positionAttribute, scene_normalAttribute, scene_colorAttribute);
        Pear.render(scene_positionAttribute, scene_normalAttribute, scene_colorAttribute);
        Cup.render(scene_positionAttribute, scene_normalAttribute, scene_colorAttribute);
        Satchel.render(scene_positionAttribute, scene_normalAttribute, scene_colorAttribute);
    }
}